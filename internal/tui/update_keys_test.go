...
func TestReservedReviewKeysStayWithLivePR(t *testing.T) {
	if !reservedReviewKey(keyPress("q")) {
		t.Fatal("q should stay with live-pr")
	}
	if reservedReviewKey(keyPress("j")) {
...
func TestPRListNavigationAndRefresh(t *testing.T) {
	...
	m = u.(Model)
	if out := ansi.Strip(m.viewContent()); !strings.Contains(out, "Pull requests") || !strings.Contains(out, "#1") {
...
func TestDetailBReturnsToPRList(t *testing.T) {
	...
	u, _ = m.Update(keyPress("b"))
	m = u.(Model)
	if m.screen != prListScreen || m.diffTerminal != nil || !strings.Contains(ansi.Strip(m.viewContent()), "Local PR") {
		t.Fatalf("b did not return to local PR list: screen=%v terminal=%v view=%q", m.screen, m.diffTerminal, ansi.Strip(m.viewContent()))
	}
}
...
func TestReviewFocusKeys(t *testing.T) {
	...
	u, _ = m.Update(keyPress("shift+tab"))
	m = u.(Model)
...
func TestPublishIsExplicitAndSingleFlight(t *testing.T) {
	...
	if _, duplicate := m.Update(keyPress("p")); duplicate != nil {
...
func TestOverloadedKeysDisambiguateByScreen(t *testing.T) {
	...
	m.ready = true
	m.w, m.h = 120, 40
	m.list = viewport.New(viewport.WithWidth(80), viewport.WithHeight(30))
	m.detail = viewport.New(viewport.WithWidth(80), viewport.WithHeight(30))
...
func TestCopyURLKeyOnListAndDetail(t *testing.T) {
	...
	if _, cmd := detail.Update(keyPress("y")); cmd == nil {
