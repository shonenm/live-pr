...
5. **TUI**: default branch/対象なしはPR一覧、current/local PRはdetailへ自動routing。一覧右paneはConversation top、metadata、CI/conflict、規模をpreview。detailは固定2paneで、`b`でlocal PRを含む一覧、他PRはcheckoutなしで開く。
