...
5. **TUI**: default branch/対象なしはPR一覧、current/local PRはdetailへ自動routing。一覧右paneは冒頭のdescription/commentをカード表示し、metadata、CI/conflict、規模をpreview。detailは固定2paneで、`b`でlocal PRを含む一覧、他PRは通常checkoutなしで開き、一覧の`c`/`x`/`m`のみ確認付きでcheckout/close/mergeする。
