...
- list: `j`/`k` selects and updates the right metadata/Conversation preview; `gg`/`G` move the PR selection to top/bottom, while `Ctrl+U`/`Ctrl+D` scroll the preview; `Enter` opens without checkout; `h`/`l` or `[`/`]` switch views; `s` changes Open/Closed/Draft; `c` checks out, `x` closes, and `m` merge-commits through a centered confirmation popup; `y` confirms and `n`/`Esc` cancels; loading work shows an animated indicator; `r` refreshes and `q` exits;
...
- detail: reserved `b` returns to the list from either pane; `j`/`k` selects Conversation items, `Ctrl+U`/`Ctrl+D` scrolls the focused Conversation, `s` changes Open/Closed/Draft, and `m` requests a merge when the current PR is open and has a verified head commit;
- `c` while the left pane is focused replaces Conversation with the local commit picker; `f` lists conflict files; `i` lists CI checks; `Esc` returns to Conversation; in static diff modes `c` toggles the selected file's check; in CodeReview, `c` toggles the selected file's reviewed check; CodeReview marks persist in Neovim user state and automatically clear when that file's `base...head` diff changes;
