...
- **Done**: default branch/対象なしではcache-firstのPR一覧、current/local PRではdetailを自動表示。`b`でlocal PRを含む一覧へ戻り、他PRはcheckoutせずnumeric pull refをfetchしてConversationとCodeReviewを表示。
