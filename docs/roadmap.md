...
- **Done**: default branch/対象なしではcache-firstのPR一覧、current/local PRではdetailを自動表示。`b`でlocal PRを含む一覧へ戻り、右previewへ冒頭のdescription/commentカード、metadata、CI/conflict/review、comments、files/lines/commitsを表示。他PRは通常checkoutせずnumeric pull refをfetchしてConversationとCodeReviewを表示し、明示的な`x`でcheckout、`m`で確認後にmerge可能。
