...
- **Done**: default branch/対象なしではcache-firstのPR一覧、current/local PRではdetailを自動表示。`b`でlocal PRを含む一覧へ戻り、右previewへConversation top、metadata、CI/conflict/review、comments、files/lines/commitsを表示。他PRはcheckoutせずnumeric pull refをfetchしてConversationとCodeReviewを表示。
